...
- {ref}`BasicsAndPrimer_Basics`
- {ref}`BasicsAndPrimer_Primer`
    - {ref}`BasicsAndPrimer_Simple_Configurations`
    - {ref}`BasicsAndPrimer_Elaborate_Configurations`
    - {ref}`BasicsAndPrimer_Dynamics_of_Execution`
    - {ref}`BasicsAndPrimer_Control`
    - {ref}`BasicsAndPrimer_Parameters`
    - {ref}`BasicsAndPrimer_Monitoring_Values`
    - {ref}`BasicsAndPrimer_Learning`
    - {ref}`BasicsAndPrimer_Customization`
...
PsyNeuLink models are made up of {class}`Components <Component>` and {class}`Compositions <Composition>`:
Components are objects that perform a specific function, and Compositions are used to combine Components into a model.
There are two primary kinds of Components: {class}`Mechanisms <Mechanism>` and {class}`Projections <Projection>`. For those
...
other Mechanisms. Projections allow the output of one Mechanism to be transmitted to another. {class}`Compositions <Composition>` combine
these Components into pathways that constitute a [computational graph](https://en.wikipedia.org/wiki/Graph_(abstract_data_type)),
in which the Mechanisms are nodes and Projections are directed edges. Compositions can also be
nodes, so that one Composition can be nested inside another to create more complex, hierarchical models (e.g., of
circuits or pathways within a larger system-level model). A {class}`Scheduler` coordinates the execution of all of the
...
{class}`Conditions <Condition>` to handle more complex structures, such as feedback pathways, and/or the execution
...
Mechanisms and Projections fall into two broad categories: {class}`ProcessingMechanisms <ProcessingMechanism>`
*directly transmit* and possibly *transform* information, and are linked by {class}`PathwayProjections <PathwayProjection>`
that transmit the information between them. {class}`ModulatoryMechanisms <ModulatoryMechanism>`
*modify* or *modulate* the transmission and transformation of information, by way of {class}`ModulatoryProjections <ModulatoryProjection>`
to the Components they modulate. PsyNeuLink provides a library of Components of
...
drift diffusion decision process); and there are {class}`ModulatoryMechanisms <ModulatoryMechanism>`, including
{class}`ControlMechanisms <ControlMechanism>` that can be used to modulate the functioning of ProcessingMechanisms, and
{class}`LearningMechanisms <LearningMechanism>` that can be used to modify Projections, respectively. The {attr}`function <Mechanism_Base.function>`
of a Mechanism defines the operation it carries out. PsyNeuLink provides a rich {ref}`library <Functions>` of mathematical,
statistical and matrix manipulation functions. However, a Mechanism can also be
assigned any Python function that is consistent with that Mechanism's type (see {ref}`BasicsAndPrimer_Customization`).
...
planned for future inclusion. The {ref}`QuickReference` provides a more detailed overview of PsyNeuLink objects and its
...
(BasicsAndPrimer_GrandView_Figure)=
...
{ref}`tutorial <Tutorial>` provides additional introductory material for those who are newer to computational modeling,
...
the first layer of which takes an array of length 5 as its input, and uses a {class}`Linear` function
(the default for a {class}`ProcessingMechanism`), and the other two of which take 1d arrays of the specified sizes and use a
...
Each of the Mechanisms can be executed individually, by simply calling its {meth}`execute <Mechanism_Base.execute>` method
...
The Composition connects the Mechanisms into a pathway that form a graph, which can be shown using its {meth}`show_graph <ShowGraph.show_graph>` method:

(BasicsAndPrimer_Simple_Pathway_Example_Figure)=
...
display and/or analysis (such as [NetworkX](https://networkx.github.io) and [igraph](http://igraph.org/redirect.html)).
They can also be exported as a JSON file, in a format that is currently being developed for the exchange
of computational models in neuroscience and psychology (see {ref}`json`).

The Composition can be run by calling its {meth}`run <Composition.run>` method, with an input array appropriately sized for
...
determines their order in the pathway. More complicated arrangements can be created by using one of the Composition's
{ref}`pathway addition methods <Composition_Pathway_Addition_Methods>`, by adding nodes individually using a
Composition's {meth}`add_nodes <Composition.add_nodes>` method, and/or by creating intersecting pathways, as shown in some
of the examples further below.

PsyNeuLink picks sensible defaults when necessary Components are not specified. In the example above no {class}`Projections <Projection>`
were actually specified, so PsyNeuLink automatically created the appropriate types (in this case,
{class}`MappingProjections <MappingProjection>`), and sized them appropriately to connect each pair of Mechanisms. Each
Projection has a {attr}`matrix <Projection_Base.matrix>` parameter that weights the connections between the elements of the
output of its {attr}`sender <Projection.sender>` and those of the input to its {attr}`receiver <Projection.receiver>`. Here, the
...
Composition's {ref}`pathway addition methods <Composition_Pathway_Addition_Methods>` is used to create the pathway, as an
...
This is a simplified version of the model described in [Cohen et al. (1990)](https://www.researchgate.net/publication/20956134_Cohen_JD_McClelland_JL_Dunbar_K_On_the_control_of_automatic_processes_a_parallel_distributed_processing_account_of_the_Stroop_effect_Psychol_Rev_97_332-361).

The figure below shows the model using the Composition's {meth}`show_graph <ShowGraph.show_graph>` method.

(BasicsAndPrimer_Simple_Stroop_Example_Figure)=
...
Running the model is as simple as generating some inputs and then providing them to the {meth}`run <Composition.run>`
...
When a Composition is run, its {attr}`results <Composition.results>` attribute stores the values of its `OUTPUT` Mechanisms
at the end of each {ref}`trial <TimeScale.TRIAL>`. In this case, the {class}`DDM` Mechanism is the only `OUTPUT` Mechanism, and it
has two output values by default: the outcome of the decision (1 or -1, in this case corresponding to `red` or
`green`), and the estimated mean decision time for the decision (in seconds). So, the value returned by the {attr}`results <Composition.results>`
attribute is a 3d array containing two 2d arrays, each of which has the two outputs of the DDM
for each {ref}`trial <TimeScale.TRIAL>` (notice that the estimated response time for the second, incongruent trial was
significantly longer than for the first, congruent trial; note also that, on some executions it might return -1 as
the response in the second trial since, by default, the {attr}`function <DDM.function>` used for the decision process has
a non-zero {attr}`noise <DriftDiffusionAnalytical.noise>` term).
...
determined by the projections between them (and shown in the {meth}`show_graph <ShowGraph.show_graph>` method). In the
`Stroop_model` above, the `decision` Mechanism executes once per pass, just after the `output` Mechanism. The
`decision` Mechanism is a {class}`DDM`. This uses {class}`DriftDiffusionAnalytical` as its default {attr}`function <DDM.function>`,
which computes an analytic solution to the distribution of responses using the DDM integration process, and returns
both the probability of crossing a specified {attr}`threshold <DriftDiffusionAnalytical.threshold>`, and the mean
crossing time. However, it is also possible to simulate the dynamics of the integration process. This can be done by
assigning {class}`DriftDiffusionIntegrator` as the Mechanism's {attr}`function <DDM.function>` and, in the call to the Composition's
{meth}`run <Composition.run>` method, specifying that a {ref}`trial <TimeScale.TRIAL>` terminates only when the `decision`
Mechanism has completed its execution, as follows:

```python
# Modify construction of decision Mechanism:
...
The output is now the result of the {class}`DriftDiffusionIntegrator`, which is the value of the decision variable when it
crosses threshold (which is, by definition, equal to either the positive or negative value of the {attr}`threshold <DriftDiffusionAnalytical.threshold>`
attribute), and the number of executions it took to do so. Since the `decision`
Mechanism is the last (`TERMINAL`) Mechanism of the Composition, it is also its `OUTPUT` Mechanism. Therefore, its
output is recorded in the {attr}`results <Composition.results>` attribute of the Stroop model, as shown above (note: because
...
it was complete. PsyNeuLink's {class}`Scheduler` can be used to implement more complicated dependencies among Mechanisms, by
creating one or more {class}`Conditions <Condition>` for execution of those Mechanisms and assigning those to the Composition's
{class}`Scheduler`. Conditions can specify the behavior of a Mechanism on its own (e.g., how many times it should be executed
in each {ref}`trial <TimeScale.TRIAL>`), its behavior relative to one or more other Components (e.g., how many times it
...
that uses a [leaky competing accumulator](https://www.ncbi.nlm.nih.gov/pubmed/11488378) ({class}`LCAMechanism`) for the
...
```python
# Modify construction of task Mechanism:
...
doing so itself. This example also imposes a fixed "settling time" (100 executions) on the `task` Mechanism. However,
...
```python
# Define a function that detects when a Mechanism's value has converged, such that the change in all of the
...
PsyNeuLink provides a rich set of {ref}`pre-defined Conditions <Condition_Pre-Specified_List>` (such as `When` in the
...
conflict in the `output` Mechanism on each {ref}`trial <TimeScale.TRIAL>`, and use that to regulate the gain of the
`task` Mechanism:
...
{class}`ControlProjections <ControlProjection>` that implement this. Furthermore, the constructor for the
{class}`ObjectiveMechanism` used in the **objective_mechanism** argument specifies that it should monitor the value of the
`output` Mechanism, and use the {class}`Energy` Function to evaluate it. PsyNeuLink automatically constructs the
MappingProjections from `output` to the ObjectiveMechanism, and from the latter to the ControlMechanism. The latter
is then added to the `Stroop_model` as its {attr}`controller <Composition.controller>` in its constructor.
The result is shown in the figure below, using the **show_controller** option of the Composition's {meth}`show_graph <ShowGraph.show_graph>` method:

(BasicsAndPrimer_Stroop_Example_With_Control_Figure)=
...
The `task` Mechanism is configured to reset at the beginning of each {ref}`trial <TimeScale.TRIAL>`, and the
**call_after_trial** argument of the Composition's {meth}`run <Composition.run>` method is used to print Mechanism values
at the end of each {ref}`trial <TimeScale.TRIAL>` (see {ref}`below <Stroop_model_output>`).

When the Composition executes, the Objective Mechanism receives the output of the `output` Mechanism, and uses the
{class}`Energy` function assigned to it to compute conflict in the `output` Mechanism (i.e., the degree of co-activity of
the `red` and `green` values). The result is passed to the `control` Mechanism, which uses it to set the {attr}`gain <Logistic.gain>`
of the `task` Mechanism's {class}`Logistic` function. The `task` Mechanism is configured to
reset at the beginning of each {ref}`trial <TimeScale.TRIAL>`; and, since the `control` Mechanism was assigned as
the Composition's {attr}`controller <Composition.controller>`, it executes at the end of each {ref}`trial <TimeScale.TRIAL>`
after all of the other Mechanisms in the Composition have executed, which has its effects on the `task` Mechanism
the next time it executes (i.e., on the next {ref}`trial <TimeScale.TRIAL>`; a Composition's {attr}`controller <Composition.controller>`
can also be configured to execute at the start of a {ref}`trial <TimeScale.TRIAL>`). Finally, the
**call_after_trial** argument of the Composition's {meth}`run <Composition.run>` method is used to print Mechanism values
at the end of each {ref}`trial <TimeScale.TRIAL>`. The **animate** argument of the {meth}`run <Composition.run>` method can be
used to generate an animation of the Composition's execution, as shown below:

(BasicsAndPrimer_Stroop_Example_Animation_Figure)=
...
with `animate={"show_controller":True}` in call to the {meth}`run <Composition.run>`.
```

Running the model, including the setup and execution:
...
Running it for four {ref}`trials <TimeScale.TRIAL>` produces the following output:
...
the model generates the incorrect response to the incongruent stimulus ([-1] = green, rather than [1] = red), due to
the stronger weights of the Projections in the `word_pathway`. However, because this is associated with a moderate
amount of conflict (`[0.51]`), control is increased on the next trial, which in turn increases the gain of the
`task` Mechanism, strengthening its representation of the instruction so that it eventually fully activates the
color task and generates the correct response. A more elaborate example of this model can be found at
{class}`BotvinickConflictMonitoringModel`. More complicated forms of control are also possible, for example, ones that run
internal simulations to optimize the amount of control to optimize some criterion (e.g., maximize the
[expected value of control](https://royalsocietypublishing.org/doi/full/10.1098/rstb.2013.0478)),
or to implement [model-based learning](https://royalsocietypublishing.org/doi/full/10.1098/rstb.2013.0478).
...
the state of its operation. For example, every Component has a {ref}`value <Component_Value>` parameter that stores the
result of the Component's {ref}`function <Component_Function>` after it has executed. (Note here the difference in the
...
any other Python attribute (as described below), they are actually instances of a special {class}`Parameters` class that
supports a number of important features. These include the ability to simultaneously have different values in
different contexts (often referred to as {ref}`"statefulness" <Parameter_Statefulness>`), the ability to keep a record of
previous values, and the ability to be {ref}`modulated <ModulatorySignal_Modulation>` by other Components in PsyNeuLink.
These features are supported by methods on the Parameter class, as described below.

(BasicsAndPrimer_Accessing_Parameters)=
#### Accessing Parameter Values

The most recently assigned value of a parameter can be accessed like any other attribute in Python,
by using "dot notation" -- that is, `<Component>.<parameter>`. For instance, the print statements in the
`print_after` function of the example above use `output.value` and `decision.value` to access the {attr}`value <Mechanism_Base.value>`
parameter of the `output` and `decision` Mechanisms, respectively (more specifically, they
access the first element of the output Mechanism's value, `output.value[0]`, and the first and second elements of the
decision Mechanism's {attr}`value <Mechanism_Base.value>`). This returns their most recently assigned values. However, as an
instance of the {class}`Parameters` class, a parameter can be {attr}`stateful <Parameter.stateful>`, which means it can have more
than one value associated with it. For example, PsyNeuLink has the capacity to execute the same Component in
different {ref}`contexts <Parameter_Statefulness>`, either as part of different Compositions or, within the same
Composition, as part of {ref}`model-based simulations <OptimizationControlMechanism_Model_Based>` executed by the
Composition's {attr}`controller <Composition_Controller>`. The value of a parameter in a particular context can be
accessed by using the {meth}`get <Parameter.get>` method for the parameter and providing the context, for example:
...
{ref}`Composition_Scope_of_Execution` for additional information about how to specify contexts. If no context is specified,
then the {meth}`get <Parameter.get>` method returns the most recently assigned value of the parameter, just like dot
notation.

If a parameter is {attr}`stateful <Parameter.stateful>`, then its previous value can also be accessed, using the
{meth}`get_previous <Parameter.get_previous>` method; for example:
...
specified to have a longer {attr}`history <Parameter.history>`, in which case {meth}`get_previous <Parameter.get_previous>` can
be used to access earlier values. For example, the following sets output Mechanism's {attr}`value <Mechanism_Base.value>`
...
If included in the script above, then the following would return the `output` Mechanism's {attr}`value <Mechanism_Base.value>`
from two trials before the last one run:
...
(BasicsAndPrimer_Setting_Parameters)=
...
- the {meth}`set <Parameter.set>` method of the Parameter class; that is
  `<Component>.parameters.<parameter>.set(<value>, <context>)`.

In the case of {ref}`non-stateful parameters <Parameter_Statefulness>`, these two approaches behave equivalently, in which
...
In the case of {ref}`stateful parameters <Parameter_Statefulness>` (the most common type), assigning a value using dot
notation and the {meth}`set <Parameter.set>` method can behave differently under some circumstances.

Dot notation is simpler, but assumes that the {class}`context <Context>` for which the specified value should apply is the
most recent context in which the Parameter's owner Component has executed. If the value should apply to a different
context, then the {meth}`set <Parameter.set>` method must be used, which allows explicit specification of the context.
See below for examples of appropriate use cases for each, as well as explanations of contexts when executing
{ref}`Mechanisms <Mechanism_Execution_Composition>` and {ref}`Compositions <Composition_Execution_Context>`, respectively.

:::{warning}
If a given component has not yet been executed, or has only been executed standalone without a user-provided context,
then dot notation will set the Parameter's value in the baseline context. Analogously, calling the {meth}`set <Parameter.set>`
method without providing a context will also set the Parameter's value in the baseline
...
The {ref}`parameters <Component_Parameters>` attribute of a Component contains a list of all of its parameters. It is
important here to recognize the difference between the parameters of a Component and those of its {ref}`function <Component_Function>`.
In the examples above, {ref}`value <Component_Value>` is a parameter of the `output` and
`decision` Mechanisms themselves. However, each of those Mechanisms also has a {attr}`function <Mechanism_Base.function>`;
and, since those are PsyNeuLink {class}`Functions <Function>` which are also Components, those too have
parameters. For example, the `output` Mechanism was assigned the {class}`Logistic` {class}`Function`, which has a {attr}`gain <Logistic.gain>`
and a {attr}`bias <Logistic.bias>` parameter (as well as others). The parameters of a Component's
{ref}`function <Component_Function>` can also be accessed using dot notation, by referencing the function in the
specification. For example, the current value of the {attr}`gain <Logistic.gain>` parameter of the `output`'s Logistic
...
a {class}`ModulatorySignal` belonging to a {class}`ModulatoryMechanism`). If the parameter
of a {class}`Mechanism` or a {class}`Projection` is modulable, it may be assigned a {class}`ParameterPort` -- this is a
...
a Mechanism, its {attr}`function <Mechanism_Base.function>`, any of its
...
returned by accessing the parameter directly (as in the examples above); see {ref}`ModulatorySignal_Modulation` for a more
complete description of modulation. The current *modulated* value of a parameter can be accessed from the {attr}`value <ParameterPort.value>`
of the corresponding ParameterPort. For instance, the print statement in the example above
used `task.parameter_ports[GAIN].value` to report the modulated value of the {attr}`gain <Logistic.gain>` parameter of
the `task` Mechanism's {class}`Logistic` function when the simulation was run. For convenience, it is also possible to
...
`modulated` refers to the {attr}`value <ParameterPort.value>` of the ParameterPort for the parameter:
...
{attr}`function <Mechanism_Base.function>`, or secondary functions. Note that,
...
This is because when the Composition was run, the `control` Mechanism modulated the value of the gain parameter.
...
{attr}`noise <TransferMechanism.noise>` is set to a function or value
...
the Composition's {meth}`run <Composition.run>` method, that calls the `print_after` function defined in Python. There
are other similar "hooks" in the {meth}`run <Composition.run>` method that can be used not only to monitor values, but also
to carry out custom operations at various points during execution (before and/or after each {ref}`run <TimeScale.RUN>`,
{ref}`trial <TimeScale.TRIAL>` or execution of the Components in a trial).
...
printed to the console using its {meth}`print_entries <Log.print_entries>` method, and specifying the desired information
...
The output of a {class}`Log` can also be reported in various other formats, including as a
[numpy](https://docs.scipy.org/doc/numpy/reference/generated/numpy.array.html) array (using its {meth}`nparray <Log.nparray>` method), as a
dictionary of values for each entry (using its {meth}`nparray_dictionary <Log.nparray_dictionary>` method), and in
[CSV](https://en.wikipedia.org/wiki/Comma-separated_values) format (using its {meth}`csv <Log.csv>` method).
...
Python-based environments. Currently, PsyNeuLink has builtin integration with [PyTorch](https://pytorch.org),
however other environments can be accessed using {class}`UserDefinedFunctions <UserDefinedFunction>`. Since such
...
PsyNeuLink has a native class -- {class}`LearningMechanism` -- that can be used to implement various forms of learning,
including unsupervised forms (such as {class}`Hebbian`) and supervised forms (such as reinforcement learning and
backpropagation). LearningMechanisms take as their input a target and/or an error signal, provided by a
{class}`MappingProjection` from the source of the error signal (either a ComparatorMechanism or another LearningMechanism).
LearningMechanisms use {class}`LearningSignals <LearningSignal>` (a type of {class}`OutputPort`) to send a {class}`LearningProjection` to the
{class}`MappingProjection` that is being learned. The type of learning implemented by a LearningMechanism is determined by
the class of {ref}`LearningFunction <LearningFunctions>` assigned as its {attr}`function <LearningMechanism.function>`. In some
...
Calling the Composition's {meth}`show_graph <ShowGraph.show_graph>` with `show_learning=True` shows the network along with
all of the learning components created by the call to `add_backpropagation_pathway`:

(BasicsAndPrimer_XOR_MODEL_Figure)=
...
(BasicsAndPrimer_Rumelhart_Model)=

The model shown above implements learning for a simple linear path. However, virtually any model can be created
using calls to a Composition's {ref}`learning methods <Composition_Learning_Methods>` to build up more complex pathways.
...
([pdf](https://web.stanford.edu/class/psych209a/ReadingsByDate/02_08/RumelhartTodd93.pdf)):
...
The figure below shows this network with all of its {ref}`learning components <Composition_Learning_Components>`:

(BasicsAndPrimer_Rumelhart_Network_Figure)=
...
{class}`LearningProjections <LearningProjection>` to them can be shown.
```

Given the number of learning components, training the model above using standard PsyNeuLink components can take a
considerable amount of time. However, the same Composition can be implemented using the {class}`AutodiffComposition`, by
replacing the relevant line in the example above with `comp = AutodiffComposition(name='Rumelhart Semantic Network')`.
The AutodiffComposition uses [PyTorch](https://pytorch.org) to execute learning, which runs considerably (as much as
three orders of magnitude) faster (see {ref}`Composition_Learning`, as well as {ref}`Composition_Learning_AutodiffComposition`
for comparisons of the advantages and disadvantages of using a standard {class}`Composition` vs. {class}`AutodiffComposition` for
learning).

(BasicsAndPrimer_Customization)=
### Customization

The Mechanisms in the examples above all use PsyNeuLink {class}`Functions <Function>`. However, as noted earlier, a Mechanism can be
...
{attr}`function <Mechanism_Base.function>` of a Mechanism (or any other Component in PsyNeuLink), it is automatically
"wrapped" as a {class}`UserDefinedFunction`, a special class of PsyNeuLink {class}`Function <Function>` that integrates it with
PsyNeuLink: in addition to making its first argument available as the input to the Component to which it is assigned,
it also makes its parameters available for modulation by {class}`ControlMechanisms <ControlMechanism>`. For example,
notice that `my_sinusoidal_fct` has two other arguments, in addition to its `input`: `phase` and `amplitude`.
As a result, the phase and amplitude of `my_wave_mech` can be modulated by referencing them in the constructor of a
...
mentioned under {ref}`BasicsAndPrimer_Learning`, functions from other environments that implement complex learning models
can be assigned as the {attr}`function <Mechanism_Base.function>` of a Mechanism, and in that way integrated into a
PsyNeuLink model.

### Conclusion

The examples above are intended to provide a sample of PsyNeuLink's capabilities, and how they can be used. The
{ref}`Tutorial <Tutorial>` provides a more thorough, interactive introduction to its use, and the {ref}`User's Guide <UserGuide>` provides
